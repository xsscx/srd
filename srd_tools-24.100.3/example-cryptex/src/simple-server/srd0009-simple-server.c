// Patch by @h02332 on January 29, 2021 for a Crash in simple-server example code
// due to passing a NULL pointer fprintf (which eventually calls "_platform_strlen"),
// causing a segmentation fault in Security Research Tools 20C80 example code.
//
//
#include <stdio.h>
#include <stdbool.h>
...
int main(int argc, char ** argv) {
    ...
    while (true) {
        ...
        if (cryptex_mount_path != NULL) {
            ...
            fprintf(stdout, "The environment variable CRYPTEX_MOUNT_PATH is not set.\n");
        }
        fflush(stdout);
        close(fd);
    }
